...
# 在 Docker 中使用 uv {#using-uv-in-docker}

## 快速入门 {#getting-started}

!!! tip

    请查看 [`uv-docker-example`](https://github.com/astral-sh/uv-docker-example) 项目，了解在 Docker 中使用 uv 构建应用的最佳实践示例。

uv 提供了 _distroless_（无发行版）Docker 镜像和基于流行基础镜像的衍生镜像。distroless 镜像适用于[将 uv 二进制文件复制](#installing-uv)到你自己的镜像构建中，而衍生镜像适用于在容器中使用 uv。distroless 镜像仅包含 uv 二进制文件，不包含其他任何内容。相比之下，衍生镜像包含预装 uv 的操作系统。
...
!!! important
    ...
    最佳实践是在仓库中添加 `.venv` 到 [`.dockerignore` 文件](https://docs.docker.com/build/concepts/context/#dockerignore-files)，以防止它被包含在镜像构建中。项目虚拟环境依赖于你的本地平台，应该在镜像中从头创建。
...
完整示例请参见 [`uv-docker-example` 项目](https://github.com/astral-sh/uv-docker-example/blob/main/Dockerfile)。

### 使用环境 {#using-the-environment}
...
!!! tip
    ...
    或者，可以在同步之前设置 [`UV_PROJECT_ENVIRONMENT` 设置](../../concepts/projects/config.md#project-environment-path)，以安装到系统 Python 环境中，从而完全跳过环境激活步骤。

### 使用已安装的工具 {#using-installed-tools}

要使用已安装的工具，请确保[工具二进制目录](../../concepts/tools.md#tool-executables)在路径中：
...
## 在容器中开发 {#developing-in-a-container}

开发时，将项目目录挂载到容器中非常有用。通过这种设置，对项目的更改可以立即反映到容器化服务中，而无需重新构建镜像。但是，重要的是_不要_将项目虚拟环境（`.venv`）包含在挂载中，因为虚拟环境是平台特定的，应该保留为镜像构建的那个。

### 使用 `docker run` 挂载项目 {#mounting-the-project-with-docker-run}

通过[匿名卷](https://docs.docker.com/engine/storage/#volumes)将项目（工作目录）绑定挂载到 `/app`，同时保留 `.venv` 目录：
...
完整示例请参见 [`uv-docker-example` 项目](https://github.com/astral-sh/uv-docker-example/blob/main/run.sh)。

### 使用 `docker compose` 配置 `watch` {#configuring-watch-with-docker-compose}

使用 Docker compose 时，可以使用更复杂的工具来进行容器开发。[`watch`](https://docs.docker.com/compose/file-watch/#compose-watch-versus-bind-mounts) 选项提供了比绑定挂载更精细的控制，并支持在文件更改时触发对容器化服务的更新。

!!! note

    此功能需要 Compose 2.22.0，该版本随 Docker Desktop 4.24 一起提供。

在你的 [Docker compose 文件](https://docs.docker.com/compose/compose-application-model/#the-compose-file)中配置 `watch`，以挂载项目目录但不同步项目虚拟环境，并在配置更改时重新构建镜像：
...
完整示例请参见 [`uv-docker-example` 项目](https://github.com/astral-sh/uv-docker-example/blob/main/compose.yml)。

## 优化 {#optimizations}

### 编译字节码 {#compiling-bytecode}
...
#### 工作区中的中间层 {#intermediate-layers-in-workspaces}

如果你使用[工作区](../../concepts/projects/workspaces.md)，则需要进行一些更改：
...
## 使用 pip 接口 {#using-the-pip-interface}
...
### 安装 requirements 文件 {#installing-requirements}
...
## 验证镜像来源 {#verifying-image-provenance}
...
GitHub 证明基于 [sigstore.dev 基础设施](https://www.sigstore.dev/)构建。因此，你也可以使用 [`cosign` 命令](https://github.com/sigstore/cosign) 来验证证明 blob 与 `uv` 的（多平台）清单：
