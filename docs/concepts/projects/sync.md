...
锁定（Locking）是将项目的依赖项解析为[锁文件](./layout.md#the-lockfile)（lockfile）的过程。同步（Syncing）是将锁文件中的一部分包安装到[项目环境](./layout.md#the-project-environment)中的过程。

## 自动锁定与同步 {#automatic-lock-and-sync}

锁定和同步在 uv 中是*自动*进行的。例如，当使用 `uv run` 时，项目会在执行所请求的命令之前先进行锁定和同步。这确保了项目环境始终保持最新。同样，读取锁文件的命令（如 `uv tree`）也会在运行前自动更新锁文件。
...
!!! important
    ...
    当新版本的包发布时，uv 不会将锁文件视为过期——如果您想升级依赖项，需要显式更新锁文件。详情请参阅[升级锁定的包版本](#upgrading-locked-package-versions)文档。

## 创建锁文件 {#creating-the-lockfile}
...
## 同步环境 {#syncing-the-environment}

虽然环境是[自动同步](#automatic-lock-and-sync)的，但也可以使用 `uv sync` 显式同步环境：
...
### 可编辑安装 {#editable-installation}

当环境被同步时，uv 会将项目（以及其他工作空间成员）安装为*可编辑*（editable）包，这样对代码的更改无需重新同步即可反映在环境中。

要退出此行为，请使用 `--no-editable` 选项。

!!! note

    如果项目未定义构建系统，则不会被安装。详情请参阅[构建系统](./config.md#build-systems)文档。

### 处理多余包 {#handling-of-extraneous-packages}
...
### 同步可选依赖 {#syncing-optional-dependencies}
...
有关如何管理可选依赖项的详细信息，请参阅[可选依赖](./dependencies.md#optional-dependencies)文档。

### 同步开发依赖 {#syncing-development-dependencies}
...
`--only-dev` 标志可用于安装 `dev` 组，而*不安装*项目及其依赖项。
...
有关如何管理开发依赖项的详细信息，请参阅[开发依赖](./dependencies.md#development-dependencies)文档。

## 升级锁定的包版本 {#upgrading-locked-package-versions}
...
使用这些选项时，目标的所有依赖项仍然会被安装。例如，`--no-install-project` 会省略*项目本身*，但不会省略其任何依赖项。

如果使用不当，这些标志可能会导致环境损坏，因为某个包可能会缺少其依赖项。

## 恶意软件检查 {#malware-checks}

!!! important

    同步时的恶意软件检查处于[预览阶段](../preview.md)，在稳定之前可能会发生变化。

在同步过程中，uv 可以通过对照 [OSV](https://osv.dev) 对锁文件进行轻量级扫描来检查已知恶意软件。OSV 引用了 OpenSSF 的[恶意包数据库](https://github.com/ossf/malicious-packages)中的 MAL 通告。
