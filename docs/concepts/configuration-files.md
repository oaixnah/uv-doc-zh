...
请参阅[设置参考](../reference/settings/project-metadata.md)了解可用设置的完整列表。
...
提供了一个专门的 [`[tool.uv.pip]`](../reference/settings/configuration.md#pip) 部分，用于配置*仅* `uv pip` 命令行接口。此部分中的设置不会应用于 `uv pip` 命名空间之外的 `uv` 命令。但是，此部分中的许多设置在顶级命名空间中都有对应的设置，这些设置*会*应用于 `uv pip` 接口，除非被 `uv.pip` 部分中的值覆盖。
